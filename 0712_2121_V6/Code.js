...
function closeSpieltagServer(spieltagId, bruttoSieger, nettoSieger)
{
    try
    {
        const ssApp = SpreadsheetApp.openByUrl(getSpreadsheetUrl("ssApp"));
        ...
        if (zeile === -1)
        {
            return { success: false, error: "Spieler-Spieltag nicht in der Datenbank gefunden." };
        }

        tSpieltage.getRange(zeile, 4).setValue("Beendet");
        tSpieltage.getRange(zeile, 6).setValue(bruttoSieger);
        tSpieltage.getRange(zeile, 7).setValue(nettoSieger);
